...
src_model = os.path.join(src_folder, 'densenet161-8d451a50.pth')
dst_model = os.path.join(model_path, 'densenet161-8d451a50.pth')
...
def train_model(model, criterion, optimizer, num_epochs=1):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch >= 50 and epoch < 80 and epoch_loss <= least_loss_50:
            ...
            if muti_gpu:
                torch.save(model.module, 'den161_50.pth')
            else:
                torch.save(model, 'den161_50.pth')
            file.copy('./den161_50.pth', 's3://' + OBS_Name + '/den161_ft/den161_50.pth')
        elif epoch >= 80 and epoch < 120 and epoch_loss <= least_loss_100:
            least_loss_100 = epoch_loss
            if muti_gpu:
                torch.save(model.module, 'den161_120.pth')
            else:
                torch.save(model, 'den161_120.pth')
            file.copy('./den161_120.pth', 's3://' + OBS_Name + '/den161_ft/den161_120.pth')
        elif epoch >= 120 and epoch < 150 and epoch_loss <= least_loss_150:
            least_loss_150 = epoch_loss
            if muti_gpu:
                torch.save(model.module, 'den201_150.pth')
            else:
                torch.save(model, 'den161_150.pth')
            file.copy('./den161_150.pth', 's3://' + OBS_Name + '/den161_ft/den161_150.pth')
        elif epoch >= 150 and epoch_loss <= least_loss_200:
            least_loss_200 = epoch_loss
            if muti_gpu:
                torch.save(model.module, 'den161_200.pth')
            else:
                torch.save(model, 'den161_200.pth')
            file.copy('./den161_200.pth', 's3://' + OBS_Name + '/den161_ft/den161_200.pth')
