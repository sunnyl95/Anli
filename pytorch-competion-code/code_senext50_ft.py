...
src_model = os.path.join(src_folder, 'se_resnext50_32x4d-a260b3a4.pth')
dst_model = os.path.join(model_path, 'se_resnext50_32x4d-a260b3a4.pth')
...
file.copy_parallel(os.path.join('s3://' + 'lxj', 'final'), file_folder)
...
model = make_model("se_resnext50_32x4d", num_classes=40, pretrained=True)
...
def train_model(model, criterion, optimizer, num_epochs=1):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch >= 50 and epoch < 80 and epoch_loss <= least_loss_50:
            ...
            if muti_gpu:
                torch.save(model.module, 'se50_50.pth')
            else:
                torch.save(model, 'se50_50.pth')
            file.copy('./se50_50.pth', 's3://' + OBS_Name + '/se50_ft_newdata/se50_50.pth')
        elif epoch >= 80 and epoch < 120 and epoch_loss <= least_loss_100:
            least_loss_100 = epoch_loss
            if muti_gpu:
                torch.save(model.module, 'se50_120.pth')
            else:
                torch.save(model, 'se50_120.pth')
            file.copy('./se50_120.pth', 's3://' + OBS_Name + '/se50_ft_newdata/se50_120.pth')
        elif epoch >= 120 and epoch < 150 and epoch_loss <= least_loss_150:
            least_loss_150 = epoch_loss
            if muti_gpu:
                torch.save(model.module, 'se50_150.pth')
            else:
                torch.save(model, 'se50_150.pth')
            file.copy('./se50_150.pth', 's3://' + OBS_Name + '/se50_ft_newdata/se50_150.pth')
        elif epoch >= 150 and epoch_loss <= least_loss_200:
            least_loss_200 = epoch_loss
            if muti_gpu:
                torch.save(model.module, 'se50_200.pth')
            else:
                torch.save(model, 'se50_200.pth')
            file.copy('./se50_200.pth', 's3://' + OBS_Name + '/se50_ft_newdata/se50_200.pth')
