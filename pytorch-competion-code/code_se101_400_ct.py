...
src_name_2 = os.path.join(src_folder, 'se101_400_43.pth')
dst_name_2 = os.path.join(dst_folder, 'se101_400_43.pth')
...
# src_model = os.path.join(src_folder, 'dpn107_extra-b7f9f4cc9.pth')
# dst_model = os.path.join(model_path, 'dpn107_extra-b7f9f4cc9.pth')
...
file.copy_parallel(os.path.join('s3://' + 'jian1299', 'origin'), file_folder)
...
model = torch.load(dst_name_2, map_location=torch.device('cpu'))
...
def train_model(model, criterion, optimizer, num_epochs=1):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch >= 50 and epoch < 80 and epoch_loss <= least_loss_50:
            ...
            if muti_gpu:
                torch.save(model.module, 'se101_ct_50.pth')
            else:
                torch.save(model, 'se101_ct_50.pth')
            file.copy('./se101_ct_50.pth', 's3://' + OBS_Name + '/se101_ct/se101_ct_50.pth')
        elif epoch >= 80 and epoch < 120 and epoch_loss <= least_loss_100:
            least_loss_100 = epoch_loss
            if muti_gpu:
                torch.save(model.module, 'se101_ct_120.pth')
            else:
                torch.save(model, 'se101_ct_120.pth')
            file.copy('./se101_ct_120.pth', 's3://' + OBS_Name + '/se101_ct/se101_ct_120.pth')
        elif epoch >= 120 and epoch < 150 and epoch_loss <= least_loss_150:
            least_loss_150 = epoch_loss
            if muti_gpu:
                torch.save(model.module, 'se101_ct_150.pth')
            else:
                torch.save(model, 'se101_ct_150.pth')
            file.copy('./se101_ct_150.pth', 's3://' + OBS_Name + '/se101_ct/se101_ct_150.pth')
        elif epoch >= 150 and epoch_loss <= least_loss_200:
            least_loss_200 = epoch_loss
            if muti_gpu:
                torch.save(model.module, 'se101_ct_200.pth')
            else:
                torch.save(model, 'se101_ct_200.pth')
            file.copy('./se101_ct_200.pth', 's3://' + OBS_Name + '/se101_ct/se101_ct_200.pth')
