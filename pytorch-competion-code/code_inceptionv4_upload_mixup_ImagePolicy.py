...
src_name_1 = os.path.join(src_folder, 'autoaugment.py')
dst_name_1 = os.path.join(dst_folder, 'autoaugment.py')
...
from modules.autoaugment import ImageNetPolicy
...
train_transforms = transforms.Compose([
        ImageNetPolicy(),
        transforms.RandomResizedCrop((299, 299), (0.6, 1.0)),
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'val' and epoch_acc > best_acc and epoch_acc > 0.9:
                best_acc = epoch_acc
                torch.save(model.module, './inceptionv4_mixup_ImagePolicy_resize.pth')
                file.copy('./inceptionv4_mixup_ImagePolicy_resize.pth', 's3://' + OBS_Name + f'/inceptionv4_mixup_ImagePolicy_resize/inceptionv4_mixup_ImagePolicy_resize_acc_{epoch_acc}.pth')
                # best_model_wt = copy.deepcopy(model.state_dict())
            if phase == 'val' and epoch_loss < least_loss:
                least_loss = epoch_loss
                torch.save(model.module, './inceptionv4_mixup_ImagePolicy_resize.pth')
                file.copy('./inceptionv4_mixup_ImagePolicy_resize.pth', 's3://' + OBS_Name + f'/inceptionv4_mixup_ImagePolicy_resize/inceptionv4_mixup_ImagePolicy_resize_loss_{epoch_loss}.pth')
