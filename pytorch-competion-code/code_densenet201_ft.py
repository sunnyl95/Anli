...
src_model = os.path.join(src_folder, 'densenet201-c1103571.pth')
dst_model = os.path.join(model_path, 'densenet201-c1103571.pth')
...
def train_model(model, criterion, optimizer, num_epochs=1):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch >= 50 and epoch < 80 and epoch_loss <= least_loss_50:
            ...
            if muti_gpu:
                torch.save(model.module, 'den201_50.pth')
            else:
                torch.save(model, 'den201_50.pth')
            file.copy('./den201_50.pth', 's3://' + OBS_Name + '/den201_ft/den201_50.pth')
        elif epoch >= 80 and epoch < 120 and epoch_loss <= least_loss_100:
            least_loss_100 = epoch_loss
            if muti_gpu:
                torch.save(model.module, 'den201_120.pth')
            else:
                torch.save(model, 'den201_120.pth')
            file.copy('./den201_120.pth', 's3://' + OBS_Name + '/den201_ft/den201_120.pth')
            ...
            if muti_gpu:
                ...
                torch.save(model, 'den201_150.pth')
            file.copy('./den201_150.pth', 's3://' + OBS_Name + '/den201_ft/den201_150.pth')
        elif epoch >= 150 and epoch_loss <= least_loss_200:
            least_loss_200 = epoch_loss
            if muti_gpu:
                torch.save(model.module, 'den201_200.pth')
            else:
                torch.save(model, 'den201_200.pth')
            file.copy('./den201_200.pth', 's3://' + OBS_Name + '/den201_ft/den201_200.pth')
