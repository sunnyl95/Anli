...
file.copy_parallel(os.path.join('s3://' + OBS_Name, 'garbage_classify_all'), file_folder)
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'val' and epoch_acc > best_acc:
                best_acc = epoch_acc
                torch.save(model.module, './eff_b4_mixup_lookahead.pth')
                file.copy('./eff_b4_mixup_lookahead.pth', 's3://' + OBS_Name + '/eff_b4_mixup_lookahead/eff_b4_mixup_lookahead.pth')
...
model = train_model(model, criterion, lookahead, num_epochs=300)
