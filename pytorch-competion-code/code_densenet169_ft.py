...
src_model = os.path.join(src_folder, 'densenet169-b2777c0a.pth')
dst_model = os.path.join(model_path, 'densenet169-b2777c0a.pth')
...
model.classifier = nn.Linear(num_ft, 40)


train_transforms = transforms.Compose([
        transforms.RandomResizedCrop(256, (0.16, 1.0)),
...
def train_model(model, criterion, optimizer, num_epochs=1):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch >= 50 and epoch < 80 and epoch_loss <= least_loss_50:
            ...
            if muti_gpu:
                torch.save(model.module, 'den169_50.pth')
            else:
                torch.save(model, 'den169_50.pth')
            file.copy('./den169_50.pth', 's3://' + OBS_Name + '/den169_ft_oridata/den169_50.pth')
        elif epoch >= 80 and epoch < 120 and epoch_loss <= least_loss_100:
            least_loss_100 = epoch_loss
            if muti_gpu:
                torch.save(model.module, 'den169_120.pth')
            else:
                torch.save(model, 'den169_120.pth')
            file.copy('./den169_120.pth', 's3://' + OBS_Name + '/den169_ft_oridata/den169_120.pth')
        elif epoch >= 120 and epoch < 150 and epoch_loss <= least_loss_150:
            least_loss_150 = epoch_loss
            if muti_gpu:
                torch.save(model.module, 'den169_150.pth')
            else:
                torch.save(model, 'den169_150.pth')
            file.copy('./den169_150.pth', 's3://' + OBS_Name + '/den169_ft_oridata/den169_150.pth')
        elif epoch >= 150 and epoch_loss <= least_loss_200:
            least_loss_200 = epoch_loss
            if muti_gpu:
                torch.save(model.module, 'den169_200.pth')
            else:
                torch.save(model, 'den169_200.pth')
            file.copy('./den169_200.pth', 's3://' + OBS_Name + '/den169_ft_oridata/den169_200.pth')
