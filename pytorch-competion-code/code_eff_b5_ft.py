...
def train_model(model, criterion, optimizer, num_epochs=1):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch >= 50 and epoch < 80 and epoch_loss <= least_loss_50:
            ...
            if muti_gpu:
                torch.save(model.module, 'eff_b5_50.pth')
            else:
                torch.save(model, 'eff_b5_50.pth')
            file.copy('./eff_b5_50.pth', 's3://' + OBS_Name + '/eff_b5_ft/eff_b5_50.pth')
        elif epoch >= 80 and epoch < 120 and epoch_loss <= least_loss_100:
            least_loss_100 = epoch_loss
            if muti_gpu:
                torch.save(model.module, 'eff_b5_120.pth')
            else:
                torch.save(model, 'eff_b5_120.pth')
            file.copy('./eff_b5_120.pth', 's3://' + OBS_Name + '/eff_b5_ft/eff_b5_120.pth')
        elif epoch >= 120 and epoch < 150 and epoch_loss <= least_loss_150:
            least_loss_150 = epoch_loss
            if muti_gpu:
                torch.save(model.module, 'eff_b5_150.pth')
            else:
                torch.save(model, 'eff_b5_150.pth')
            file.copy('./eff_b5_150.pth', 's3://' + OBS_Name + '/eff_b5_ft/eff_b5_150.pth')
        elif epoch >= 150 and epoch_loss <= least_loss_200:
            least_loss_200 = epoch_loss
            if muti_gpu:
                torch.save(model.module, 'eff_b5_200.pth')
            else:
                torch.save(model, 'eff_b5_200.pth')
            file.copy('./eff_b5_200.pth', 's3://' + OBS_Name + '/eff_b5_ft/eff_b5_200.pth')
