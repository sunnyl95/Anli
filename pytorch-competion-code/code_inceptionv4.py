...
src_model = os.path.join(src_folder, 'inceptionv4-8e4777a0.pth')
dst_model = os.path.join(model_path, 'inceptionv4-8e4777a0.pth')
...
file.copy_parallel(os.path.join('s3://jian1299/garbage_data/Ftrain'), file_folder_train)
file.copy_parallel(os.path.join('s3://jian1299/garbage_data/Ftest'), file_folder_test)
...
save_file_name = 'inceptionv4'
# ###################################


model = pretrainedmodels.inceptionv4(pretrained='imagenet')
model.last_linear = nn.Linear(1536, 40)

train_transforms = transforms.Compose([
        transforms.RandomResizedCrop((299, 299), (0.2, 1.0)),
...
def data_flow(train_data_dir, val_data_dir, batch_size):
    ...
    val_loader = DataLoader(dataset=validation_set, batch_size=batch_size, num_workers=4, shuffle=False)
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'train' and epoch_loss <= train_loss:
                ...
                if muti_gpu:
                    ...
                    file.copy('./' + save_file_name + '.pth', 's3://' + OBS_Name + '/' + save_file_name + '/' + save_file_name + f'_train_loss_{epoch_loss}_dict.pth')
                else:
            ...
            if phase == 'val' and epoch_loss <= least_loss:
                ...
                if muti_gpu:
                    torch.save(model.module, './' + save_file_name + '.pth')
                    file.copy('./' + save_file_name + '.pth', 's3://' + OBS_Name + '/' + save_file_name + '/' + save_file_name + f'_val_loss_{epoch_loss}_dict.pth')
                else:
                    torch.save(model, './' + save_file_name + '.pth')
                    file.copy('./' + save_file_name + '.pth', 's3://' + OBS_Name + '/' + save_file_name + '/' + save_file_name + f'_val_loss_{epoch_loss}_dict.pth')
            ...
            if phase == 'val' and epoch_acc >= best_acc and epoch_acc >= 0.905:
                ...
                if muti_gpu:
                    torch.save(model.module, './' + save_file_name + '.pth')
                    file.copy('./' + save_file_name + '.pth', 's3://' + OBS_Name + '/' + save_file_name + '/' + save_file_name + f'_acc_{epoch_loss}_dict.pth')
                else:
                    torch.save(model, './' + save_file_name + '.pth')
                    file.copy('./' + save_file_name + '.pth', 's3://' + OBS_Name + '/' + save_file_name + '/' + save_file_name + f'_acc_{epoch_loss}_dict.pth')
...
train_set, validation_set, train_loader, val_loader = data_flow(train_data_dir=file_folder_train, 
                                                                val_data_dir=file_folder_test,
