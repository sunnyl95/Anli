    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'val' and epoch_acc > best_acc and epoch_acc > 0.9:
                best_acc = epoch_acc
                torch.save(model.module, './eff_b4_mixup_imagepolicy_size.pth')
                file.copy('./eff_b4_mixup_imagepolicy_size.pth', 's3://' + OBS_Name + f'/eff_b4_mixup_imagepolicy_size/eff_b4_mixup_imagepolicy_size_acc_{epoch_acc}.pth')
                # best_model_wt = copy.deepcopy(model.state_dict())
            if phase == 'val' and epoch_loss < least_loss:
                least_loss = epoch_loss
                torch.save(model.module, './eff_b4_mixup_imagepolicy_size.pth')
                file.copy('./eff_b4_mixup_imagepolicy_size.pth', 's3://' + OBS_Name + f'/eff_b4_mixup_imagepolicy_size/eff_b4_mixup_imagepolicy_size_loss_{epoch_loss}.pth')
