    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            if phase == 'train' and epoch_loss <= train_loss:
                ...
                if muti_gpu:
                    torch.save(model.module, './resnext50.pth')
                    file.copy('./resnext50.pth', 's3://' + OBS_Name + f'/resnext50/resnext50_train_loss_{epoch_loss}.pth')
                else:
                    torch.save(model, './resnext50.pth')
                    file.copy('./resnext50.pth', 's3://' + OBS_Name + f'/resnext50/resnext50_train_loss_{epoch_loss}.pth')
            ...
            if phase == 'val' and epoch_loss <= least_loss:
                ...
                if muti_gpu:
                    torch.save(model.module, './resnext50.pth')
                    file.copy('./resnext50.pth', 's3://' + OBS_Name + f'/resnext50/resnext50_val_loss_{epoch_loss}.pth')
                else:
                    torch.save(model, './resnext50.pth')
                    file.copy('./resnext50.pth', 's3://' + OBS_Name + f'/resnext50/resnext50_val_loss_{epoch_loss}.pth')
            ...
            if phase == 'val' and epoch_acc >= best_acc and epoch_acc >= 0.905:
                ...
                if muti_gpu:
                    torch.save(model.module, './resnext50.pth')
                    file.copy('./resnext50.pth', 's3://' + OBS_Name + f'/resnext50/resnext50_acc_{epoch_acc}.pth')
                else:
                    torch.save(model, './resnext50.pth')
                    file.copy('./resnext50.pth', 's3://' + OBS_Name + f'/resnext50/resnext50_acc_{epoch_acc}.pth')
