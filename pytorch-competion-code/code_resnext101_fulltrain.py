...
file.copy_parallel(os.path.join('s3://' + 'jian1299', 'all'), file_folder)
...
def train_model(model, criterion, optimizer, num_epochs=1):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch_loss <= train_loss:
            ...
            if muti_gpu:
                torch.save(model.module, './resnext101_fulltrain.pth')
                file.copy('./resnext101_fulltrain.pth', 's3://' + OBS_Name + f'/resnext101_fulltrain/resnext101_fulltrain_train_loss_{epoch_loss}.pth')
            else:
                torch.save(model, './resnext101_fulltrain.pth')
                file.copy('./resnext101_fulltrain.pth', 's3://' + OBS_Name + f'/resnext101_fulltrain/resnext101_fulltrain_train_loss_{epoch_loss}.pth')
