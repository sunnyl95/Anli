...
file_folder = 'xxxxxxxxxx'
# ###################################

model = make_model('se_resnext101_32x4d', num_classes=40, pretrained=True)
...
def train_model(model, criterion, optimizer, num_epochs=1):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch >= 50 and epoch < 80 and epoch_loss <= least_loss_50:
            ...
            if muti_gpu:
                torch.save(model.module, 'senext101_50.pth')
            else:
                torch.save(model, 'senext101_50.pth')
            ...
            if muti_gpu:
                torch.save(model.module, 'senext101_150.pth')
            else:
                torch.save(model, 'senext101_150.pth')
        elif epoch >= 150 and epoch_loss <= least_loss_200:
            least_loss_200 = epoch_loss
            if muti_gpu:
                torch.save(model.module, 'senext101_200.pth')
            else:
                torch.save(model, 'senext101_200.pth')
